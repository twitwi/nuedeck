...
// idea: / as a separator?
// idea: instead of %+, %..., have either a function call like, or @+ @...? (check for minimal conflict with css etc)
let parseAnim = txt => {
...
export default () => ({
  ...
  /*async*/ enrichGeneratedSlidesHeader({ type, headerLines }) {
    ...
    for (let ih in headerLines) {
      let header = headerLines[ih]
      let txt = null
      let pre = null
