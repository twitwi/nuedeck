...
cp public/test.css "$o"/nuedeck/nuedeck-theme.css
...
cp node_modules/katex/dist/katex.min.css "$o"/nuedeck
if ! has no-katex-fonts ; then
    cp node_modules/katex/dist/fonts/* "$o"/nuedeck/fonts
fi
ok

# TODO: also produce a font pack with katex and all styles
